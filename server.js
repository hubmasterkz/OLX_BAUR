/**
 * HUB MASTER — BAUR / Бауржан (ремонт бытовой техники + электрика, только Алматы)
 * Схема работы: Wazzup webhook остается на Нурике/общем роутере.
 * Нурик/роутер пересылает сообщения канала BAUR на этот сервер: POST /webhook или POST /relay/wazzup.
 * Этот BAUR-сервер НЕ регистрирует Wazzup webhook и НЕ перебивает Нурика.
 * BAUR сам отвечает клиенту через свой WAZZUP_CHANNEL_ID и сам отправляет [ЗАЯВКА] / [ПОЗВОНИТЬ] / отказ в CRM.
...
// Код бота для БД/CRM. Для CRM лучше короткий код "baur".
const BOT_NAME          = process.env.BOT_NAME || "baur";

// Источник заявки и брендинг для Telegram / CRM
const SOURCE_NUMBER = process.env.SOURCE_NUMBER || "OLX BAUR";
const SHOP_NAME     = process.env.SHOP_NAME || "HUB MASTER";
const REPORT_NAME   = process.env.REPORT_NAME || "HUB MASTER · OLX BAUR";

// CRM webhook (отправка заявок в Google Apps Script CRM)
const CRM_WEBHOOK_URL    = process.env.CRM_WEBHOOK_URL || "";
const CRM_WEBHOOK_SECRET = process.env.CRM_WEBHOOK_SECRET || "";
const CRM_SEND_UPDATES   = String(process.env.CRM_SEND_UPDATES || "true").toLowerCase() === "true";

// Опциональная защита, если Нурик/роутер будет передавать секрет в header X-BAUR-RELAY-SECRET.
// Если пусто — пересылка принимается без проверки секрета.
const BAUR_RELAY_SECRET  = process.env.BAUR_RELAY_SECRET || process.env.RELAY_SECRET || "";
const INTAKE_MODE        = process.env.BAUR_INTAKE_MODE || "nurik_relay";
...
function buildCrmLeadPayload(phone, raw, dailyNumber, globalId, eventType) {
  ...
  const type = eventType || "new_lead";
  const externalId = globalId
    ? `${BOT_NAME}_${type}_${globalId}`
    : `${BOT_NAME}_${type}_${phoneClean}_${dailyNumber}_${Date.now()}`;

  return {
    secret: CRM_WEBHOOK_SECRET,

    // Источник — чтобы CRM записала в колонку "Источник" как OLX Baur.
    source: SOURCE_NUMBER,
    leadSource: SOURCE_NUMBER,
    sourceNumber: SOURCE_NUMBER,
    utmSource: SOURCE_NUMBER,
    integrationSource: "bot_webhook",

    // Код бота — чтобы CRM понимала, что это BAUR, и не путала с Нуриком / Абдуллой.
    sourceBot: BOT_NAME,
    bot: BOT_NAME,
    botName: "BAUR",
    shopName: SHOP_NAME,
    ...
    eventType: type,
    status: type === "refusal" ? "Отказ/Завершено" : "Новая",
    globalId: globalId || null,
    externalId,
    botLeadId: globalId ? String(globalId) : "",
    dailyNumber,
...
async function sendLeadToCrm(phone, raw, dailyNumber, globalId, eventType = "new_lead") {
  if (!CRM_WEBHOOK_URL) {
    ...
    return { ok: false, skipped: true, reason: "CRM_WEBHOOK_URL_EMPTY" };
  }
  ...
  try {
    ...
    console.log(`✅ CRM ${eventType}: ${payload.externalId} отправлено, статус ${resp.status}`);
    return { ok: true, status: resp.status, externalId: payload.externalId, crmResponse: resp.data };
  } catch (e) {
    const err = e.response?.data || e.message;
    console.error(`❌ CRM ${eventType} send error:`, err);
    return { ok: false, error: err, externalId: payload.externalId };
  }
}
...
async function saveCallback(phone, replyText) {
  ...
  sendLeadToCrm(phone, cleanBody, todayCount, globalIdCb, "callback").catch(e => console.error("CRM callback async error:", e.message));
}
...
// ── Webhook (Wazzup через Нурика/роутер) ─────────────────────
const messageBuffers = {};
const processedIds = new Set();
const BUFFER_MS = 10000;

function extractWazzupMessages(body) {
  body = body || {};
  if (Array.isArray(body.messages)) return body.messages;
  if (body.message && typeof body.message === "object") return [body.message];
  if (body.data && Array.isArray(body.data.messages)) return body.data.messages;
  if (body.payload && Array.isArray(body.payload.messages)) return body.payload.messages;
  if (body.payload && body.payload.message && typeof body.payload.message === "object") return [body.payload.message];
  // Иногда Нурик может переслать сразу объект одного сообщения.
  if (body.chatId || body.text || body.messageId || body.channelId) return [body];
  return [];
}

function isRelayAuthorized(req) {
  if (!BAUR_RELAY_SECRET) return true;
  const got = String(
    req.get("X-BAUR-RELAY-SECRET") ||
    req.get("X-Relay-Secret") ||
    req.query.secret ||
    (req.body && (req.body.relaySecret || req.body.secret)) ||
    ""
  );
  return got === BAUR_RELAY_SECRET;
}

async function processIncomingWebhook(req, res, sourceLabel) {
  if (!isRelayAuthorized(req)) {
    console.warn(`⛔ ${sourceLabel}: неверный relay secret`);
    return res.status(403).json({ ok: false, error: "BAD_RELAY_SECRET" });
  }

  res.status(200).json({ ok: true, bot: BOT_NAME, source: SOURCE_NUMBER, mode: INTAKE_MODE });

  try {
    const body = req.body || {};
    if (body.test === true) return;
    const messages = extractWazzupMessages(body);
    if (!messages.length) {
      console.log(`⚠️ ${sourceLabel}: messages не найдены`);
      return;
    }
    ...
    console.error(`${sourceLabel} webhook error:`, err.stack || err.message || err);
  }
}

// Основной URL, куда Нурик/роутер должен пересылать сообщения BAUR-канала.
app.post("/webhook", (req, res) => processIncomingWebhook(req, res, "BAUR /webhook"));
app.post("/relay/wazzup", (req, res) => processIncomingWebhook(req, res, "BAUR /relay/wazzup"));
app.post("/nurik/webhook", (req, res) => processIncomingWebhook(req, res, "BAUR /nurik/webhook"));
app.post("/relay", (req, res) => processIncomingWebhook(req, res, "BAUR /relay"));
...
app.get("/health", (req, res) => {
  res.json({
    ...
    wazzupConfigured: Boolean(WAZZUP_API_KEY && WAZZUP_CHANNEL_ID),
    telegramConfigured: Boolean(process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_CHAT_ID),
    databaseConfigured: Boolean(process.env.DATABASE_URL),
    relaySecretEnabled: Boolean(BAUR_RELAY_SECRET),
    now: new Date().toISOString()
  });
});

app.get("/crm/test", async (req, res) => {
  ...
  const raw = [
    ...
    "Телефон: +77000000001",
    "Город: Алматы",
    "Адрес: тестовая проверка BAUR → CRM",
    "Направление: Сервис",
    "Услуга: Электрика",
    "Время: по согласованию с менеджером",
    "Комментарий: Тестовая заявка из BAUR /crm/test. После проверки можно удалить."
  ].join("\n");
  const result = await sendLeadToCrm("77000000001", raw, 999, "test_" + Date.now(), "new_lead");
...
// ── ВАЖНО ПО WAZZUP ─────────────────────────────────────────
// BAUR НЕ регистрирует webhook в Wazzup. Общий webhook остается на Нурике/роутере.
// Нурик/роутер должен пересылать сообщения BAUR-канала на этот сервер: POST /webhook или POST /relay/wazzup.
...
initDB().then(async () => {
  ...
  console.log(`🔗 Режим входящих: ${INTAKE_MODE}. Wazzup webhook НЕ регистрируем — ждем пересылку от Нурика/роутера на /webhook или /relay/wazzup.`);
}).catch(err => {
